...
| **B5** | Monitor externo: sem ele, ninguem sabe que o Foton caiu. | **NAO SERVE COMO MONITOR - medido em 2026-08-29.** O Actions **esta** habilitado e verde (a afirmacao anterior de que a aba nao aparecia estava errada, e foi repetida sem verificar). Mas o cron diz `*/5` (5 min) e a **cadencia real medida pela API e de ~5 HORAS**: as duas execucoes agendadas ficaram **302 min** uma da outra. O GitHub despriorizza cron em repo publico. Uma queda no meio do evento dela seria notada **horas depois**. Consertados, ainda assim, dois furos reais: vigiava **so o duckdns** (com `app.foton.app.br` fora do ar ficava verde) e o passo do WhatsApp podia ser pulado **em silencio**. **Decisao do dono:** (a) monitor externo de verdade (UptimeRobot / Healthchecks.io - gratis, checa a cada 5 min, alerta real; exige criar conta, o agente nao cria) ou (b) a **segunda VM gratis da Oracle** pingando a primeira. O Actions fica como rede de seguranca, nao como monitor. **Falta tambem provar que o WhatsApp chega:** Actions -> Run workflow -> marcar **testar_alerta**. |
